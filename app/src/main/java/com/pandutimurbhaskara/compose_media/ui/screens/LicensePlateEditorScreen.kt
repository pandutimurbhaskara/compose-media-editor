...
import com.pandutimurbhaskara.compose_media.viewmodel.LicensePlateBlurUiState
import com.pandutimurbhaskara.compose_media.viewmodel.LicensePlateBlurViewModel

/**
 * Screen for editing license plates with blur effects
 * Detects and blurs vehicle registration plates
 */
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun LicensePlateEditorScreen(
    imageUri: Uri,
    viewModel: LicensePlateBlurViewModel = viewModel(),
    onSave: (Bitmap) -> Unit,
    onCancel: () -> Unit
) {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = { Text("Blur License Plates") },
                navigationIcon = {
                ...
                actions = {
                    // Auto detect button
                    IconButton(
                        onClick = { viewModel.detectLicensePlates() },
                        enabled = uiState is LicensePlateBlurUiState.ImageLoaded
                    ) {
        ...
        bottomBar = {
            when (val state = uiState) {
                is LicensePlateBlurUiState.PlatesDetected -> {
                    PlateBlurControlPanel(
                ...
                is LicensePlateBlurUiState.BlurApplied -> {
                    SavePanel(
        ...
        Box(
            ...
            when (val state = uiState) {
                is LicensePlateBlurUiState.Loading -> {
                    CircularProgressIndicator(
                        modifier = Modifier.align(Alignment.Center)
                    )
                }
                is LicensePlateBlurUiState.ImageLoaded -> {
                    ImageCanvas(bitmap = state.bitmap)
                }
                is LicensePlateBlurUiState.Detecting -> {
                    Column(
                        modifier = Modifier.align(Alignment.Center),
                ...
                is LicensePlateBlurUiState.Processing -> {
                    Column(
                        modifier = Modifier.align(Alignment.Center),
                        horizontalAlignment = Alignment.CenterHorizontally
                    ) {
                ...
                is LicensePlateBlurUiState.Error -> {
                    Column(
                        modifier = Modifier.align(Alignment.Center),
                        horizontalAlignment = Alignment.CenterHorizontally
                    ) {
                        ...
                        Spacer(Modifier.height(Spacing.medium))
                        Text(
                            text = "Tip: Ensure license plates are clearly visible and not at extreme angles",
...
/**
 * Display image with bounding boxes overlay for detected license plates
 */
@Composable
...
/**
 * Control panel for license plate blur settings
 */
@Composable
private fun PlateBlurControlPanel(
    blurRegions: List<BlurRegion>,
    ...
    Surface(
        ...
        Column(modifier = Modifier.padding(Spacing.medium)) {
            Text(
                text = "${blurRegions.size} license plate${if (blurRegions.size != 1) "s" else ""} detected",
            ...
            Text(
                text = "Vehicle registration plates found",
                style = MaterialTheme.typography.bodySmall,
                color = MaterialTheme.colorScheme.onSurfaceVariant
            )
...
private fun SavePanel(
    ...
    Surface(
        ...
        Column(modifier = Modifier.padding(Spacing.medium)) {
            Text(
                text = "License plates blurred successfully!",
                style = MaterialTheme.typography.titleMedium
            )
            ...
            Text(
                text = "Vehicle information is now protected",
                style = MaterialTheme.typography.bodySmall,
                color = MaterialTheme.colorScheme.onSurfaceVariant
            )
