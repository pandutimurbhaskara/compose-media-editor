...
import com.pandutimurbhaskara.compose_media.viewmodel.FaceBlurUiState
import com.pandutimurbhaskara.compose_media.viewmodel.FaceBlurViewModel

/**
 * Screen for editing faces with blur effects
 */
@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun FaceBlurEditorScreen(
    imageUri: Uri,
    viewModel: FaceBlurViewModel = viewModel(),
    onSave: (Bitmap) -> Unit,
    onCancel: () -> Unit
) {
    ...
    Scaffold(
        ...
        bottomBar = {
            when (val state = uiState) {
                is FaceBlurUiState.FacesDetected -> {
                    BlurControlPanel(
                ...
                is FaceBlurUiState.BlurApplied -> {
                    SavePanel(
        ...
        Box(
            ...
            when (val state = uiState) {
                is FaceBlurUiState.Loading -> {
                    CircularProgressIndicator(
                        modifier = Modifier.align(Alignment.Center)
                    )
                }
                is FaceBlurUiState.ImageLoaded -> {
                    ImageCanvas(bitmap = state.bitmap)
                }
                is FaceBlurUiState.Detecting -> {
                    Column(
                        modifier = Modifier.align(Alignment.Center),
...
/**
 * Control panel for blur settings
 */
@Composable
private fun BlurControlPanel(
    blurRegions: List<BlurRegion>,
    ...
    Surface(
        ...
        Column(modifier = Modifier.padding(Spacing.medium)) {
            Text(
                text = "${blurRegions.size} face${if (blurRegions.size != 1) "s" else ""} detected",
...
private fun SavePanel(
    ...
    Surface(
        ...
        Column(modifier = Modifier.padding(Spacing.medium)) {
            Text(
                text = "Blur applied successfully!",
                style = MaterialTheme.typography.titleMedium
            )
